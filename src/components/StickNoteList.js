...
const StickyNoteList = ({notes, handleAddNote, handleDeleteNote, handleChangeColour}) => {
    return (
        <>
            <div className="grid grid-cols-4 md:grid-cols-2 sm:grid-cols-1 gap-4 mr-auto ml-auto max-w-8xl pl-4 pr-4 pt-4 pb-4 text-dark">
                ...
                {notes.map((note) => <StickyNote date={note.date} text={note.note} color={note.colour} id={note.id} handleChangeColour={handleChangeColour} handleDeleteNote={handleDeleteNote}/>)}
